...
module.exports = [
    {
        ...
        handler: async (request, h) => {
            const { favsService, userService , filmService} = request.services(); // userService pour récupérer l'utilisateur
            ...
            try {
                ...
                const findFilm = await filmService.findById(film_id);

                if (findFav) {
                    return h.response({ message: 'Favoris déjà ajouté' }).code(400);
                }if (!findFilm) {
                    return h.response({ message: 'Film introuvable' }).code(404);
                }
    ...
    },
    {
        ...
        handler: async (request, h) => {
            ...
            try {
                ...
                if (!findFav) {
                    return h.response({ message: 'Favoris inexistant' }).code(400);
                }

                const fav = await favsService.delete(findFav.id);
                return h.response(fav).code(201);
            } catch (error) {
                return h.response({ message: 'Erreur interne' }).code(500);
            }
